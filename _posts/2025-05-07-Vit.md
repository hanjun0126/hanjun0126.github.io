...
1.   $\mathbf{z'}_l=\mathbf{MSA}(\mathbf{LN}(\mathbf{z_{l-1}})),\ l=1\cdots L$
2.   $\mathbf{z}_l=\mathbf{MLP}(\mathbf{LN}(\mathbf{z'}_l))+\mathbf{z'}_l,\ l=\ 1\cdots L$

​	▸ LayerNormalization을 사용한다. 이는 토큰의 개별적으로 작동할 수 있기 때문이다.
...
실험을 통해서 ResNet, ViT, 그리고 하이브리드 모델의 성능을 평가하였다. 각 모델이 
...
<figure>
    ...
    <figcaption:text-algin:center;>Table 1: Details of Vision Transformer model variants.</figcaption></p>
</figure>
...
7.   ▸ ResNet(BiT): Batch Normalization 대신 Group Normalization 을 사용하고, 표준화된 합성곱을 적용한 모델
...
<figure>
    <p align="center"><img src="../assets/gitbook/images/Vision_Transformer/Table_2.png" style="zoom:50%;" />
    <figcaption:text-align:center;>Table 2: Comparison with state of the art on popular image classification benchmarks.</figcaption></p>
</figure>
