...
필자들은 NLP에서 지배적으로 쓰이는 Transformer를 컴퓨터 비전 분야에서 사용하기 위한 연구를 진행하였다. 이전에 연구들은 self-attention을 CNN에 결합시키거나 일부를 대체하는 형태였고, 크게 성과를 얻지는 못했다. 이에 이미지를 패치들로 나누고 각 패치를 NLP에서 사용되는 토큰처럼 활용하여 Transformer의 입력으로 주는 방향성으로 연구가 진행됐다. Transformer는 CNN에 내제된 inductive bias가 없기에 대규모의 데이터가 아니면 기존의 ResNet 모델들보다 낮은 성능을 보였다. 하지만 대규모의 데이터셋에서 사전 학습을 하고, 전이 학습을 했을 때는 SOTA에 근접하거나 이를 능가했다.



---

**ViT(Vision Transformer)**는 2020년 구글의 "An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale"논문을 통해 소개된 이미지 인식을 위한 딥러닝 모델이다. ViT는 이미지 분류 모델에서 사용되는 CNN 모델의 합성곱 계층 방법을 사용하지 않고 트랜스포머 모델에서 사용되는 셀프 어텐션(self-attention)을 적용한다. 합성곱 모델은 이미지를 분류하기 위해 지역 특징을 추출했다면 ViT는 셀프 어텐션을 사용해 전체 이미지를 한 번에 처리하는 방식으로 구현됐다.
...
ViT는 Transformer처럼 이미지를 패치 단위로 나누어 1차원 토큰 임베딩 시퀀스를 입력으로 받는다. 2차원 이미지 $\mathbf{x}\in\mathbb{R}^{H\times W\times C}$를 2차원 패치 $\mathbf{x}_p \in \mathbb{R}^{N \times (P^2\cdot C)}$ 로 변형하여 인코더에 입력으로 들어가는 과정을 보자.

<img src="../assets/gitbook/images/Vision_Transformer/Figure_1_split_the_image.png" style="zoom:50%;" />

1.   $\mathbf{x}\in\mathbb{R}^{H\times W\times C}$&rarr;$\mathbf{x}_p \in \mathbb{R}^{N \times (P^2\cdot C)}$ : 2차원 이미지 $\mathbf{x}$가 $N$개의 패치 $\mathbf{x_p}$로 나누어진다. 패치 크기가 클수록 $N$은 작아지기에 토큰 수가 줄어든다.

2.   각각의 패치들은 1차원 벡터로 펼쳐지면서 선형 투영된다. &rarr; $\mathbf{x}_p^i\mathbf{E}\in\mathbb{R}^D$ : $\mathbf{x}_p^i\in\mathbb{R}^{P^2\cdot C}$ $\times$ $\mathbf{E}\in\mathbb{R}^{(P^2\cdot C)\times D}$  $+\ \alpha$(bias)
...
▸ $\mathbf{z}_0 = [\mathbf{x}_{class};\ \mathbf{x}_p^1\mathbf{E};]$
...
​	▸ Appendix D.4 에서 학습 가능한 위치 임베딩에 대한 설명을 확인
...
자연어 처리에 사용되는 BERT 모델과 ViT 모델은 트랜스포머 모델의 구조를 그대로 사용하지만, 입력 데이터를 만드는 과정이 서로 다르다. BERT는 문장보다 작은 단위의 토큰들이 순차적으로 입력되면 ViT 모델은 이미지가 작은 단위의 이미지 패치로 나뉘어 순차적으로 입력된다는 차이가 있다. 



#### 합성곱 모델과 ViT 모델 비교

합성공 신경망과 트랜스포머는 이미지 특징을 잘 표현하는 임베딩을 만들고자 하는 목적은 같지만, 그 과정에는 큰 차이가 있다. 합성곱 신경망의 임베딩은 이미지 패치 중 일부만 선택하여 학습하며, 이를 통해 이미지 전체의 특징을 추출한다.
...
필자들은 JFT-300M 데이터셋에서의 전이 성능을 기준으로 다양한 모델들의 스케일링 특성을 통제된 조건에서 비교 분석한다. 이 설정에서는 데이터 크기가 모델 성늠의 병목이 되지 않기 때문에, 각 모델의 사전 학습 비용 대비 성능을 평가할 수 있다. 모델 구성은 다음과 같다.
