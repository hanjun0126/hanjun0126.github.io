...
ViT의 MSA기반 모듈을 shift window 기반의 모듈로 대체하여 Swin Transformer 블록이 설계되었다. 블록의 구성은 shift window 기반의 MSA 모듈과 2개의 GELU를 이용한 MLP 계층이 있다. LayerNorm 계층이 각각의 모듈과 계층 앞에 적용되었고, residual connection이 연결되어 있다.
