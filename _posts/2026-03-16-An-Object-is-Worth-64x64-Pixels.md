...
<img src="../assets/post/post2/fig3.png"/>
...
즉 3D point 들이 2D 좌표를 가지는 과정이다. UV-island 또는 UV-chart라고 부르는 $I_i=(S_i,f_i(S_i))$ 는 3D 패치와 2D 공간에서의 대응 영역을 나타낸다. 이 $N$ 개의 모든 UV-islands들의 집합을 $\mathcal M$ 의 UV-atlas라고 한다. UV공간에서 어떤 점이 실제 surface에 해당하는지를 표시하는 함수 occupancy function $\alpha$ 는 다음과 같다.
...
Fig. 3의 왼쪽은 representation 을 보여준다. 3D 객체(mesh) $\mathcal M$ 은 분해된 패치 {$S_i$} 에서 uv-map $f_i$ 를 통해 2D로 펼쳐진다. 그 결과 두 가지 이미지가 생성된다. Geometry image (R, R, 4) 와 Material image (R, R, 8) 은 합쳐져서 omage의 구성이 되고, omage는 다시 3D mesh로 remeshing 될 수 있다. 

$$\text{Geometry image (R, R, 4)}+\text{Material image (R, R, 8)}\rightarrow O^*$$

$$O^* = \operatorname{concat}(G, T) \in \mathbb{R}^{R \times R \times 12}$$$



### Generative Modeling for omages

Omage 생성은 이미지 생성과 집합 생성의 성질을 동시에 가진다. 패치 내부는 이미지처럼 grid 구조(regular connectivity)이기 떄문에 일반 이미지 생성 문제와 같고, 패치들의 2D 위치는 3D 구조에 영향을 주지 않기 때문에, 패치의 순서나 위치가 바뀌어도 동일한 3D로 표현할 수 있는 집합 생성 문제로 볼 수 있다. 다만 3D에서는 붙어 있지만 2D에서는 멀리 떨어질 수 있다. 3D 에서는 인접한 패치가 2D에서는 멀리 떨어질 수 있어, 이를 모델링하기 위해 long-range dependency가 필요하고, Diffusion Transformer를 사용하여 이러한 문제를 해결하였다.



### Obtaining object images

UV 맵을 가진 3D 객체는 겹치는 영역, 경계를 벗어나는 UV, 서로 맞닿은 경계, 혹은 지나치게 많은 패치와 같은 문제들 때문에 이미지로 직접 변환할 수 없다. 이 문제를 해결하기 위해, UV-atlas repacking 방법을 사용하여, material 맵과 함께 패치들을 (1024, 1024, 12) 크기의 omage로 겹치지 않도록 패치들을 재배치하여 하나의 정규 격자 이미지로 패킹한다. 패치 수가 너무 많아지는 것을 방지하기 위해, 3D 좌표와 2D UV 좌표가 동일한 정점들을 병합하고, 가장 큰 K개의 패치만 유지한다. 효율적인 학습을 위해 sparse pooling을 사용하여 이미지를 다운샘플링하며, 이 과정에서 경계를 정렬하고 빈 공간을 제거한다.

<img src="../assets/post/post2/fig4.png"/>

일반적인 다운샘플링은 경계가 흐려져, patch 사이에 gap이 생기는 문제가 발생한다. 경계를 기준으로 다시 맞춰 이를 해결한다. 고행상도 이미지에서 경계를 찾고 저해상도에서도 그 경계를 유지하도록 픽셀을 조정한다. Patch 구조를 결정하는 경계가 중요하기 때문이다. MCGIM의 mesh 기반 정확한 경계 보다 정확도는 떨어지지만 훨씬 간단하고 실용적이다.
