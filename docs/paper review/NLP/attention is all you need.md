...
**Recurrent 모델<sup>[1](#footnote_1)</sup>들은 일반적으로 입력과 출력 sequence 의 위치를 따라 계산을 수행**한다. 본질적으로 순차적인 계산 특성은 **병렬화를 배제**한다. 이는 sequence 의 길이가 길어질수록 치명적이다. &rarr; sequential computation 의 제약이 있다.

**attention** 기법은 입력과 출력 sequence 의 **거리에 상관없이 종속성을 모델링**할 수 있다.
