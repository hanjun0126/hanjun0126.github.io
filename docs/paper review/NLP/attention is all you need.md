...
입력 Q, K 는 d_k, V 는 d_v 의 차원을 가지고 $Attention(Q, K, V) = softmax({QK^t\over\sqrt{d_K}})V$ 을 만족한다. dot-product(multiplicative) attention<sup>[2](#footnote_2)</sup>에 scaling factor(${1\over\sqrt{d_K}}$) 가 추가된 알고리즘이고, 이를 scaled dot-product attention 이라고 부른다. 
...
<img src="../../../assets/images/scale_dot_product_attention.png" alt="scale_dot_product_attention" style="zoom:25%;" align="center"/>
